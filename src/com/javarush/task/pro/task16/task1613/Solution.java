...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        for (LocalDateTime dateTime : dateTimeList) {
            System.out.println(dateTime + ", is weekend - " + isWeekend(dateTime) + " " + dateTime.getDayOfWeek());
        }

    }

    public static boolean isWeekend(LocalDateTime dateTime) {
        if ((dateTime.getDayOfWeek().ordinal() + 1) < FRIDAY ) {
            return false;
        }
        if ((dateTime.getDayOfWeek().ordinal() + 1) == FRIDAY &&
        ...
        if ((dateTime.getDayOfWeek().ordinal() + 1) == SUNDAY &&
